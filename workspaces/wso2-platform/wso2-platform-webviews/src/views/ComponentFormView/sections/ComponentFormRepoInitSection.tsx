...
export const ComponentFormRepoInitSection: FC<Props> = ({ onNextClick, organization, form, initializingRepo }) => {
    ...
    const { data: gitCredentials = [], isLoading: isLoadingGitCred } = useQuery({
        ...
        queryFn: () =>
            ChoreoWebViewAPI.getInstance()
                .getChoreoRpcClient()
                .getCredentials({ orgId: organization?.id?.toString(), orgUuid: organization.uuid }),
    ...
    useEffect(() => {
        if (
            ...
            (form.getValues("credential") || !gitCredentials.some((item) => item.id === form.getValues("credential")))
        ) {
    ...
    const repoUrl =
        matchingOrgItem && repoName && buildGitURL(matchingOrgItem?.orgHandler, repoName, provider, false, serverUrl);
    useEffect(() => {
        if (
            gitOrgs?.gitOrgs.length > 0 &&
            (form.getValues("org") === "" || !gitOrgs?.gitOrgs.some((item) => item.orgName === form.getValues("org")))
        ) {
    ...
    useEffect(() => {
        if (
            ...
            !matchingOrgItem?.repositories?.some((item) => item.name === form.getValues("repo"))
        ) {
    ...
    const { data: branches = [], isLoading: isLoadingBranches } = useGetGitBranches(
        ...
        !errorFetchingGitOrg,
        {
            enabled:
                ...
                !!provider &&
                !!repoUrl &&
                (provider === GitProvider.GITHUB ? !errorFetchingGitOrg : !!credential),
            refetchOnWindowFocus: true,
        }
    );
    ...
    return (
        <>
            <div className="grid gap-4 md:grid-cols-2" ref={compDetailsSections}>
                <label className="col-span-full mb-2 opacity-80">
                    Your integration must exist in a remote Git repository in order to continue
                </label>
                <Dropdown
                    label="Git Provider"
                    ...
                    <Dropdown
                        ...
                        onChange={(e) => {
                            ...
                            if (credential === createNewCredText) {
                                form.setValue("credential", "");
                                ChoreoWebViewAPI.getInstance().openExternalChoreo(
                                    `organizations/${organization.handle}/settings/credentials`
                                );
                            } else {
                ...
                {(provider === GitProvider.GITHUB || credential) && (
                    <>
                        <Dropdown
                            ...
                            loading={
                                loadingGitOrgs ||
                                (provider === GitProvider.GITLAB_SERVER ? isLoadingGitlabCreds : false)
                            }
                            onChange={(e) => {
                                const value = (e.target as HTMLSelectElement).value;
                        ...
                        <div className="relative">
                            ...
                            <div
                                className={classNames(
                                    "w-full flex-col bg-vsc-panel-background",
                                    creatingRepo ? "absolute bottom-0 left-0 z-50 flex" : "hidden"
                                )}
