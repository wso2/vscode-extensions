...
function findSpecificGit(path: string, onValidate: (path: string) => boolean): Promise<IGit> {
	return new Promise<IGit>((c, e) => {
		...
		child.on("exit", (code) =>
			code ? e(new Error("Not found")) : c({ path, version: parseVersion(Buffer.concat(buffers).toString("utf8").trim()) }),
		);
	});
}
		...
		const onSpawn = (child: cp.ChildProcess) => {
			...
			lineStream.on("data", (line: string) => {
				...
				if ((match = /Counting objects:\s*(\d+)%/i.exec(line))) {
					...
					totalProgress = 10 + Math.floor(Number.parseInt(match[1]) * 0.1);
				} else if ((match = /Receiving objects:\s*(\d+)%/i.exec(line))) {
					totalProgress = 20 + Math.floor(Number.parseInt(match[1]) * 0.4);
				} else if ((match = /Resolving deltas:\s*(\d+)%/i.exec(line))) {
					totalProgress = 60 + Math.floor(Number.parseInt(match[1]) * 0.4);
				}
				...
				if (match !== null) {
					...
					try {
						const networkPath = await new Promise<string | undefined>((resolve) =>
							realpath.native(`${letter}:\\`, { encoding: "utf8" }, (err, resolvedPath) => resolve(err !== null ? undefined : resolvedPath)),
						);
						if (networkPath !== undefined) {
							...
							return path.normalize(
								repoUri.fsPath.replace(networkPath, `${letter.toLowerCase()}:${isDriveRoot || networkPath.endsWith("\\") ? "\\" : ""}`),
							);
						}
					} catch {}
				}
...
export class Repository {
	...
	private async handleCommitError(commitErr: any): Promise<void> {
		...
		if (/Aborting commit due to empty commit message/.test(commitErr.stderr || "")) {
	...
	async getRefs(query: RefQuery, cancellationToken?: CancellationToken): Promise<Ref[]> {
		...
		const fn = (line: string): Ref | null => {
			...
			if ((match = /^refs\/remotes\/([^/]+)\/([^ ]+) ([0-9a-f]{40}) ([0-9a-f]{40})?$/.exec(line))) {
				return { name: `${match[1]}/${match[2]}`, commit: match[3], type: RefType.RemoteHead, remote: match[1] };
			}
			if ((match = /^refs\/tags\/([^ ]+) ([0-9a-f]{40}) ([0-9a-f]{40})?$/.exec(line))) {
	...
	async getRemoteRefs(remote: string, opts?: { heads?: boolean; tags?: boolean; cancellationToken?: CancellationToken }): Promise<Ref[]> {
		...
		const fn = (line: string): Ref | null => {
			...
			if ((match = /^([0-9a-f]{40})\trefs\/tags\/([^ ]+)$/.exec(line))) {
	...
	async getStashes(): Promise<Stash[]> {
		...
		const rawStashes = result.stdout
			...
			.filter((g) => !!g)
			.map(([, index, description]: RegExpExecArray) => ({ index: Number.parseInt(index), description }));
	...
	async getCommitTemplate(): Promise<string> {
		try {
			...
			let templatePath = result.stdout.trim().replace(/^~([^\/]*)\//, (_, user) => `${user ? path.join(path.dirname(homedir), user) : homedir}/`);
