...
export class HurlBinaryManager {
	...
	async resolveCommandPath(options: ResolveCommandOptions = {}): Promise<string> {
		const configuredPath = vscode.workspace.getConfiguration('api-tryit').get<string>('hurl.path', '').trim();
		...
		const autoInstall = options.autoInstall ?? vscode.workspace.getConfiguration('api-tryit').get<boolean>('hurl.autoInstall', true);
		if (!autoInstall) {
			return 'hurl';
		}
		...
		try {
			...
			if (options.promptOnFailure) {
				...
				const action = await vscode.window.showErrorMessage(
					`${message}. You can set api-tryit.hurl.path or run "API TryIt: Install Hurl".`,
					'Install Hurl',
					'Open Settings'
				);
				...
				if (action === 'Open Settings') {
					await vscode.commands.executeCommand('workbench.action.openSettings', 'api-tryit.hurl.path');
				}
			}
			return 'hurl';
		}
	...
	async installManagedHurl(options: InstallOptions = {}): Promise<string> {
		...
		if (!asset) {
			throw new Error(
				`Managed Hurl install is not supported on ${getPlatformArchKey()}. Set api-tryit.hurl.path manually.`
			);
		}
			...
			return vscode.window.withProgress(
				{
					...
					title: 'API TryIt: Installing Hurl',
					cancellable: false
				},
	...
	private getManagedVersion(): string {
		const configuredVersion = vscode.workspace.getConfiguration('api-tryit').get<string>('hurl.version', DEFAULT_HURL_VERSION).trim();
		return configuredVersion || DEFAULT_HURL_VERSION;
	}
