    ...
    const loadSemanticDiff = useCallback(async () => {
        try {
            ...
            try {
                ...
                console.warn("[ReviewMode] Could not fetch affected packages, using current project:", error);
            }

            // Use affected packages if available, otherwise fallback to current project
            const packagesToReview = fetchedPackages.length > 0 ? fetchedPackages : [projectPath];
            ...
            const semanticDiffResponse = isMultiPackage
                ...
                : await fetchSemanticDiff(rpcClient, projectPath);
            ...
            const flowViews = semanticDiffResponse.semanticDiffs.map((diff) => {
                ...
                let belongsToPackage = projectPath;
