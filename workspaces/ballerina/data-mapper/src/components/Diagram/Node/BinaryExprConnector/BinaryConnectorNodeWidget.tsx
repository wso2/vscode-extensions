...
import { BinaryConnectorNode } from './BinaryConnectorNode';
...
export interface BinaryConnectorNodeWidgetWidgetProps {
    node: BinaryConnectorNode;
    engine: DiagramEngine;
}

export function BinaryConnectorNodeWidget(props: BinaryConnectorNodeWidgetWidgetProps) {
    ...
    return (!node.hidden && (
            <div className={classes.root} data-testid={`link-connector-node-${node?.targetPort?.getName()}`}>
                <div className={classes.header}>
                    <div>
                        {renderPortWidget(engine, node.lhsInPort, `${node?.value}-lhs-input`)}
                        {renderPortWidget(engine, node.rhsInPort, `${node?.value}-rhs-input`)}
                    </div>
