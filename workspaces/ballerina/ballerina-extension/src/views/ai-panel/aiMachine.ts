...
import { AIMachineStateValue, AIPanelPrompt, AIMachineEventType, AIMachineContext, AIUserToken, LoginMethod, AIMachineSendableEvent } from '@wso2/ballerina-core';
import { AiPanelWebview } from './webview';
import { checkToken, logout, validateApiKey, initiateInbuiltAuth } from './utils';
...
const getTokenAfterAuth = async () => {
    ...
    return { token: result.token, loginMethod: result.loginMethod };
};
...
function isExtendedEvent<K extends AIMachineEventType>(
