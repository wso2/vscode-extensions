...
export class CodeLensProviderVisitor implements Visitor {
    ...
    public beginVisitServiceDeclaration(node: ServiceDeclaration, parent?: STNode): void {
        if (node.expressions.length > 0) {
            ...
            if (
                (STKindChecker.isExplicitNewExpression(expr) &&
                    expr.typeDescriptor &&
                ...
                this.createTryItCodeLens(
                    node.position,
                    node.serviceKeyword.position,
                    node.absoluteResourcePath.map((path) => path.value).join(""),
                    node.expressions.map((exp) => exp.source.trim()).join(",")
                );
    ...
    private createVisulizeCodeLens(range: any, position: any) {
        const codeLens = new CodeLens(new Range(range.startLine, range.startColumn, range.endLine, range.endColumn));
        codeLens.command = {
            title: "Visualize",
    ...
    private createTryItCodeLens(range: any, position: any, basePath: string, listener: string) {
        const codeLens = new CodeLens(
            new Range(position.startLine, position.startColumn, position.endLine, position.endColumn)
        );
