...
const buildStatusItem = window.createStatusBarItem(
...
export async function handleOpenFile(
  ...
  const selectedPath =
    ballerinaExtInstance.getFileDownloadPath() || defaultDownloadsPath;
...
export async function handleOpenRepo(
  ...
  try {
    ...
    if (ALLOWED_ORG_LIST.includes(username)) {
      ...
      const repoLocation = await getRepositoryLocation(
...
async function selectFileDownloadPath() {
  const folderPath = await window.showOpenDialog({
    title: "Ballerina extension downloads directory",
...
async function handleSameWorkspaceFileOpen(
  ballerinaExtInstance: BallerinaExtension,
  filePath: string
) {
...
function setNextStartingUpFile(
  ballerinaExtInstance: BallerinaExtension,
  selectedPath
) {
  ballerinaExtInstance.context.globalState.update(
    NEXT_STARTING_UP_FILE,
    selectedPath
  ); // NEXT_STARTING_UP_FILE
}

// Function to open the stored cloned file path from the global state
export async function openClonedTempFile(
  ballerinaExtInstance: BallerinaExtension
) {
  ...
  const pathValue = ballerinaExtInstance.context.globalState.get(
    NEXT_STARTING_UP_FILE
  ) as string;
  if (isRepo && pathValue) {
    try {
...
async function isRepositoryLocation(ballerinaExtInstance: BallerinaExtension) {
  ...
  let repositoryLocations: Record<string, string> | undefined =
    ballerinaExtInstance.context.globalState.get(REPO_LOCATIONS);
  const workspaceFolders = workspace.workspaceFolders;
  if (workspaceFolders && workspaceFolders.length > 0) {
    const workspaceFolder = workspaceFolders[0];
    const workspaceFolderPath = workspaceFolder.uri.fsPath;
...
function isPathInRepositoryLocations(
  pathToCheck: string,
  repositoryLocations: Record<string, string> | undefined
): boolean {
...
async function resolveModules(
  langClient: ExtendedLangClient | WebExtendedLanguageClient,
  pathValue
) {
    ...
    await window.withProgress(
      ...
      async (progress, cancellationToken) => {
        ...
        const dependenciesResponse =
          await langClient.resolveMissingDependencies({
            documentIdentifier: {
              uri: uriString,
            },
          });
...
export async function handleResolveMissingDependencies(
  ballerinaExtInstance: BallerinaExtension
) {
  openClonedTempFile(ballerinaExtInstance);
    ...
    languages.onDidChangeDiagnostics(async (e) => {
      ...
      if (activeEditor && activeEditor.document.languageId === "ballerina") {
        ...
        if (
          ...
          diagnostics.filter((diag) => diag.code === "BCE2003").length > 0
        ) {
...
async function setRepositoryLocation(
  ballerinaExtInstance: BallerinaExtension,
  gitUrl: string,
  location: string
) {
...
async function _removeLocation(
  ballerinaExtInstance: BallerinaExtension,
  gitUrlHash: string
) {
  let repositoryLocations: Record<string, string> | undefined =
...
async function getRepositoryLocation(
  ballerinaExtInstance: BallerinaExtension,
  gitUrl: string
  ...
  let repositoryLocations: Record<string, string> | undefined =
    ballerinaExtInstance.context.globalState.get(REPO_LOCATIONS);
  const gitUrlHash = urlToUniqueID(gitUrl);
  const filePath: string | undefined = repositoryLocations
    ? repositoryLocations[gitUrlHash]
    : undefined;
...
export async function goToSource(
  ...
  let editor = window.visibleTextEditors.find(
    (editor) => editor.document.uri.fsPath === documentUri
  );
