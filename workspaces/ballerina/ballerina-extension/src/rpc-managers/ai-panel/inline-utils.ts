...
function transformIOType(input: IOType): FormField {
  ...
  // Handle arrays
  if (input.kind === TypeKind.Array && input.member) {
...
function transformInputs(inputs: IOType[]): {
  ...
  inputs.forEach((input) => {
    ...
    const createParameterField = (input: IOType): ParameterField => {
      ...
      if (isArrayType) {
        if (input.member) {
          // For arrays, set type to the member's kind + []
          type = `${input.member.kind}[]`;
        } else {
          type = `${typeName}`;
        }
      } else {
    ...
    const createMemberConfig = (input: IOType): FieldMetadata => {
      if (!input.typeName) {
        throw new Error("TypeName is missing");
      }
      return {
        typeName: input.kind,
        type: input.kind,
        members: input.members?.reduce((acc, member) => ({
          ...acc,
          [member.id]: {
            typeInstance: input.id,
            typeName: member.typeName || member.kind,
            type: member.kind,
            nullable: false,
            optional: input.optional || false,
          }
    ...
    if (input.category === InputCategory.Enum) {
      enums[input.id] = createMemberConfig(input);
      return;
    }
    ...
    if (input.category === InputCategory.Parameter) {
      ...
      if (input.member && input.member.fields) {
...
function transformOutput(output: IOType): FormField[] {
  if (output.kind === TypeKind.Array && output.member) {
...
export async function cleanAndGetInlineParamDefinitions(
  request: ExpandedDMModel
): Promise<{ cleanedInlineDatamapperResponse: InlineDataMapperModelResponse; parameterDefinitions: ParameterMetadata }> {
  const cleanedInlineDatamapperResponse: InlineDataMapperModelResponse = {
    mappingsModel: cleanExpandedDMModel(request)
  };

  const result = await getInlineParamDefinitions(cleanedInlineDatamapperResponse);
  const parameterDefinitions = (result as ParameterDefinitions).parameterMetadata;

  return { cleanedInlineDatamapperResponse, parameterDefinitions };
}

function transformCodeObjectToMappings(codeObject: Record<string, string>, request: InlineDataMapperModelResponse): Mapping[] {
...
async function getInlineDatamapperCode(inlineDataMapperResponse: InlineDataMapperModelResponse, parameterDefinitions: ParameterMetadata): Promise<Record<string, string>> {
  ...
  try {
    ...
    console.log("Generated Ballerina Code:", JSON.stringify(finalCode));
    return finalCode;
  } catch (error) {
    console.error(error);
    throw error;
...
export async function processInlineMappings(
  ...
  let { cleanedInlineDatamapperResponse, parameterDefinitions } = await cleanAndGetInlineParamDefinitions(request);

  if (file) {
    const mappedResult = await mappingFileInlineDataMapperModel(file, cleanedInlineDatamapperResponse);
    cleanedInlineDatamapperResponse = mappedResult as InlineDataMapperModelResponse;
  }

  const codeObject = await getInlineDatamapperCode(cleanedInlineDatamapperResponse, parameterDefinitions);
  if (!codeObject || Object.keys(codeObject).length === 0) {
    throw new Error("No valid fields were identified for mapping between the given input and output records/arrays.");
  }
  const mappings: Mapping[] = transformCodeObjectToMappings(codeObject, cleanedInlineDatamapperResponse);
  return { mappings };
}
