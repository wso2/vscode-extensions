...
/**
 * Returns the static, reusable content for the DataMap skill.
 * Used in the agent system prompt so Claude applies these rules directly
 * when generating mapping expressions — no sub-call to Claude needed.
 *
 * @param diagnosticsToolName - Exact name of the diagnostics tool the agent must call
 *   (injected by the skill file to avoid a cross-layer import).
 */
export function getDataMappingSkillContent(diagnosticsToolName: string): string {
...
Ballerina data mapping functions **must** use the expression body syntax with \`=>\`. The value after \`=>\` is a **record constructor expression**, not a code block.
...
- **Always** use \`=>\` — never write a block-body mapping function (\`function transform(...) returns Student { ... }\`).
