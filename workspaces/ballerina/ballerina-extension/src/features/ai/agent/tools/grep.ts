...
const BASE_RG_ARGS = [
    '--engine', 'default',
    '-C', '2',               // show 2 lines of context before and after each match
    '--heading',             // print the file name once as a header, not on every line
    '--line-number',         // prefix each output line with its line number
    '--ignore-file', '.gitignore', // respect .gitignore rules when scanning files
];
...
export function createGrepExecute(
    ...
    return async (input: GrepInput): Promise<GrepResult> => {
        ...
        eventHandler({
            ...
            toolInput: { pattern, path: searchPath, glob: globPattern }
        });

        console.log(`[GrepTool] Searching for pattern: "${pattern}" in ${searchPath || '.'}, glob: ${globPattern || 'default'}`);
