...
export function sendAgentDidOpen(tempProjectPath: string, projectPath: string, filePath: string): void {
  try {
    const tempFilePath = path.join(tempProjectPath, filePath);
    if (!fs.existsSync(tempFilePath)) {
      console.warn(`[AgentNotification] File does not exist, skipping didOpen: ${tempFilePath}`);
      return;
    }

    const projectFilePath = path.join(projectPath, filePath);
    if (!fs.existsSync(projectFilePath)) {
      console.warn(`[AgentNotification] File does not exist, skipping didOpen: ${projectFilePath}`);
      return;
    }
    ...
    const projectFileUri = Uri.file(projectFilePath).toString();
...
export function sendAgentDidChange(tempProjectPath: string, projectPath: string, filePath: string): void {
  try {
    const tempFilePath = path.join(tempProjectPath, filePath);
    if (!fs.existsSync(tempFilePath)) {
      console.warn(`[AgentNotification] File does not exist, skipping didChange: ${tempFilePath}`);
      return;
    }

    const projectFilePath = path.join(projectPath, filePath);
    if (!fs.existsSync(projectFilePath)) {
      console.warn(`[AgentNotification] File does not exist, skipping didChange: ${projectFilePath}`);
      return;
    }
