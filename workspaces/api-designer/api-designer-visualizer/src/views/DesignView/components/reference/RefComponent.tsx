...
const Dropdown = styled.div<ContainerProps>`
    ...
    position: absolute;
    top: ${(props: ContainerProps) => (props.$top != null ? `${props.$top}px` : 'auto')};
    left: ${(props: ContainerProps) => (props.$left != null ? `${props.$left}px` : 'auto')};
    width: fit-content;
    z-index: 1001;
...
export const RefComponent: React.FC<RefComponentProps> = (props: RefComponentProps) => {
    ...
    const dropdownTop =
        valueContainerPosition == null
            ? undefined
            : valueContainerPosition.bottom + window.scrollY + 4 + 200 > window.innerHeight
              ? valueContainerPosition.top + window.scrollY - componnetHeight - 5
              : valueContainerPosition.bottom;
    const dropdownLeft =
        valueContainerPosition == null
            ? undefined
            : valueContainerPosition.right + 200 > window.innerWidth
              ? valueContainerPosition.right - dropdownWidth
              : valueContainerPosition.left;
