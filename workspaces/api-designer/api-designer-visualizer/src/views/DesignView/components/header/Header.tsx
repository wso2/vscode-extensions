...
const HeaderAIButton = styled(AIButton)`
    && {
        padding: 4px 8px;
    }
`;
...
export function Header(props: HeaderProps) {
    const { id, header, name, headerTypes = BaseTypes, showExample = true, onRemoveHeader: onRemoveParameter, onHeaderChange: onHeaderChange } = props;
    ...
    return (
        <HorizontalFieldWrapper>
            ...
            {showExample && (
                <TextField
                    id={`headerExample-${name}`}
                    placeholder="Example"
                    value={header.schema?.example || ''}
                    sx={{ width: "30%" }}
                    onBlur={(evt) => handleHeaderChange({ 
                        ...header, 
                        schema: { 
                            ...header.schema, 
            ...
            <ButtonWrapperParams>
                <Tooltip content="Make this header optional/required">
